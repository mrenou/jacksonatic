...
public class ClassProcess {
        ...
        ((Optional<ConstructorMapping>) classMapping.getConstructorMapping()).ifPresent(constructorMapping -> {
            if (constructorMapping.isStaticFactory()) {
                List<AnnotatedMethod> methods = annotatedClass.getStaticMethods().stream()
                        .map(annotatedMethod -> {
                            boolean match = matchConstructor(classMapping, constructorMapping, annotatedMethod);
                            if (match) {
                                ...
                                IntStream.range(0, constructorMapping.getParameters().size()).forEach(index -> {
                                    JacksonaticJsonProperty jsonProperty = new JacksonaticJsonProperty(constructorMapping.getParameters().get(index).getJsonProperty(), true, index, "");
                ...
                List<AnnotatedConstructor> constructors = annotatedClass.getConstructors().stream()
                        .map(annotatedConstructor -> {
                            boolean match = matchConstructor(classMapping, constructorMapping, annotatedConstructor);
                            if (match) {
                                AnnotationMap annotationMap = new AnnotationMap();
                                annotationMap.add(new JacksonaticJsonCreator());
                                IntStream.range(0, constructorMapping.getParameters().size()).forEach(index -> {
    ...
    private static boolean matchConstructor(ClassMapping classMapping, ConstructorMapping constructorMapping, AnnotatedWithParams annotatedWithParams) {
        boolean match = false;
